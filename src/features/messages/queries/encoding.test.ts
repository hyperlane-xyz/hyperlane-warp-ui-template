...
describe('buildMessagesByOriginTxQuery', () => {
  ...
  test('builds an origin transaction query for hex transaction hashes', () => {
    const metadata = { protocol: ProtocolType.Ethereum } as ChainMetadata;

    expect(buildMessagesByOriginTxQuery(`0x${'ab'.repeat(32)}`, 8453, metadata)?.variables).toEqual(
      {
        originTxHash: `\\x${'ab'.repeat(32)}`,
        originDomainId: 8453,
      },
    );
  });
