    ...
    async (params: {
      ...
      try {
        ...
        const approveData = encodeFunctionData({
          ...
          args: [SWAP_CONTRACTS.icaBridgeRouteBase as `0x${string}`, amount],
        });
        ...
        const transferRemoteData = encodeFunctionData({
          ...
          args: [SWAP_CHAINS.origin.domainId, recipientBytes32, amount],
        });
        ...
        const hash = await params.signer.sendTransaction({
          account,
          to: SWAP_CONTRACTS.icaRouterArb as `0x${string}`,
          data: callRemoteData,
