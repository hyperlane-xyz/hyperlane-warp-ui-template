...
describe('isValidMultiCollateralToken', () => {
  test('returns false if originToken has no collateralAddressOrDenom', () => {
  ...
  test('returns false if originToken is not collateralized', () => {
    const token = createMockToken({ standard: TokenStandard.CosmosIbc });
    expect(isValidMultiCollateralToken(token, 'destination')).toBe(false);
  });

  test('returns false if destinationToken is not found via chain name', () => {
    const token = createMockToken({ connections: [createTokenConnectionMock()] });
    expect(isValidMultiCollateralToken(token, 'destination')).toBe(false);
  });

  test('returns false if destinationToken has no collateralAddressOrDenom', () => {
  ...
  test('returns false if destinationToken is not collateralized', () => {
    const token = createMockToken({
  ...
  test('returns true when tokens are valid with destinationToken as a string', () => {
    const token = createMockToken({
  ...
  test('returns true when tokens are valid with destinationToken as a IToken', () => {
    const token = createMockToken({
      connections: [createTokenConnectionMock()],
    });
