...
export async function prepareRouteTransaction(
  ...
  if (!isChainRouteTx(tx)) return deserializeSdkTransaction(tx);
...
export async function prepareApprovalTransaction({
  ...
  if (!isEVMLike(protocol)) throw new Error(`Approvals are unsupported for ${protocol}`);
  ...
  if (!populated.to || !populated.data) throw new Error('Invalid approval transaction');
  return {
    type: getRawRouteProviderType(protocol),
    transaction: { to: populated.to, data: populated.data, value: populated.value ?? '0' },
    category: 'transfer',
  } as TypedTransaction;
}
